...
#include "ui_zxhkcontroldialog.h"
...
std::list<std::string> ZXhkControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZXhkControlDialog::listMints;

ZXhkControlDialog::ZXhkControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZXhkControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZXhkControlDialog::~ZXhkControlDialog()
{
    delete ui;
}

void ZXhkControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZXhkControlDialog::updateList()
{
...
void ZXhkControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZXhkControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZXhk_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZXhkControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZXhkControlDialog::GetSelectedMints()
{
...
void ZXhkControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
