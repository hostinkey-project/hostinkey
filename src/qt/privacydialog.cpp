                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zXHK ought to be enough for anybody." - Bill Gates, 2017
    ui->zXHKpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzXHKSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzXHK->setEnabled(false);
        ui->pushButtonMintzXHK->setToolTip(tr("zXHK is currently disabled due to maintenance."));

        ui->pushButtonSpendzXHK->setEnabled(false);
        ui->pushButtonSpendzXHK->setToolTip(tr("zXHK is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zXHKpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzXHK_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXHK is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zXHK...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zXHK in ") +
...
void PrivacyDialog::on_pushButtonSpendzXHK_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXHK is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zXHK
        sendzXHK();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zXHK
    sendzXHK();
}

void PrivacyDialog::on_pushButtonZXhkControl_clicked()
{
    ZXhkControlDialog* zXhkControl = new ZXhkControlDialog(this);
    zXhkControl->setModel(walletModel);
    zXhkControl->exec();
}

void PrivacyDialog::setZXhkControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzXhkSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzXHK()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Hostinkey Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zXHKpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zXHK is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " XHK </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zXHK</b>";
    ...
    // use mints from zXhk selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZXhkControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZXhkControlDialog::GetSelectedMints();
    }
    ...
    // Clear zxhk selector in case it was used
    ZXhkControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zXhk Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zXHK </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zXHK "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zXHK "));
    ui->labelzXHKAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzXHKSyncStatus->setVisible(fShow);
}
