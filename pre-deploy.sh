...
if [[ $HOST = "i686-w64-mingw32" ]]; then
  ARCHIVE_NAME="xhk-windows-32bit.zip"
elif [[ $HOST = "x86_64-w64-mingw32" ]]; then
    ARCHIVE_NAME="xhk-windows-64bit.zip"
    ...
    ARCHIVE_NAME="xhk-linux-x64.tar.gz"
    ARCHIVE_CMD="tar -czf"
...
if [[ $HOST = "x86_64-apple-darwin11" ]]; then
    find $TRAVIS_BUILD_DIR -type f | grep -i Hostinkey-network-v1.dmg$ | xargs -i cp {} $OUTDIR/bin
else
    ...
    cp $TRAVIS_BUILD_DIR/src/hostinkey-cli $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/hostinkey-cli.exe $OUTDIR/bin/
